--TEST--
Test function php_getdns_wire2rr_dict_scan() by calling it with its expected arguments
--FILE--
<?php
...
function php_echo_addresses($dict, $callbackType, $userArg, $transID)
{
    switch ($callbackType) {
      case GETDNS_CALLBACK_COMPLETE:
        ...
        $wireDict = 0;
        $wire = str_pad($wire, $wireSize + 4, "+");
        $wireSize = $wireSize + 4;
        $result = php_getdns_wire2rr_dict_scan($wire, $wireSize, $wireDict);
        var_dump($result, $wire, $wireSize, $wireDict);
        php_getdns_dict_destroy($wireDict);
      break;
