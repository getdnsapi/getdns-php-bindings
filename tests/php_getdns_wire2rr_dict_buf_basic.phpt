--TEST--
Test function php_getdns_wire2rr_dict() by calling it with its expected arguments
--FILE--
<?php
...
function php_echo_addresses($dict, $callbackType, $userArg, $transID)
{
    switch ($callbackType) {
      case GETDNS_CALLBACK_COMPLETE:
        ...
        $wireDict = 0;
        $result = php_getdns_wire2rr_dict_buf($wire, $wireSize, $wireDict);
        var_dump($result, $wireSize, $wireDict);
