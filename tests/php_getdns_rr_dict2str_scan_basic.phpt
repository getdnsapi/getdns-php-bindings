--TEST--
Test function php_getdns_rr_dict2str_scan() by calling it with its expected arguments
--FILE--
<?php
...
function php_echo_addresses($dict, $callbackType, $userArg, $transID)
{
    switch ($callbackType) {
      case GETDNS_CALLBACK_COMPLETE:
        ...
        $result = php_getdns_rr_dict2wire_scan($reply, $str, $strLen);
        var_dump($result, $str, $strLen);
      break;
