--TEST--
Test function php_getdns_rr_dict2wire_scan() by calling it with its expected arguments
--FILE--
<?php
...
function php_echo_addresses($dict, $callbackType, $userArg, $transID)
{
    switch ($callbackType) {
      case GETDNS_CALLBACK_COMPLETE:
        ...
        $wire = "";
        $wireSize = 128;
        $wire = str_pad($wire, $wireSize);
        $result = php_getdns_rr_dict2wire_scan($reply, $wire, $wireSize);
